...
PROJECTFILE = $(or $(wildcard $(SRC_DIR)/project*.cpp $(SRC_DIR)/$(EXECUTABLE).cpp), $(SRC_DIR)/main.cpp)
...
REMOTE_PATH := eecs281/$(EXECUTABLE)_sync
#######################
# TODO (end) #
#######################
...
TESTSOURCES = $(wildcard $(TEST_DIR)/test*.cpp)
...
FULL_SUBMITFILES=$(filter-out $(TESTSOURCES), \
                   $(wildcard Makefile $(SRC_DIR)/*.h $(SRC_DIR)/*.hpp $(SRC_DIR)/*.cpp $(TEST_DIR)/test*.txt))
